...
| `PDF export fails` | Puppeteer needs Chromium; on Linux install system deps for headless Chrome |
| `AI features unavailable` | Set `GROQ_API_KEY` in `.env` or leave as `none` for fallback text |
| `Frontend build errors` | Run `npm install` in `frontend/`; use Node 18+ |
